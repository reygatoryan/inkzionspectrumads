async function initInkzionApp() {
  ...
  if (contactForm) {
    ...
    // Facebook dropdown: robust toggle, focus management, outside click and keyboard support
    (function setupFbDropdown(){
      const fbBtn = document.getElementById('fb-btn');
      if (!fbBtn) return;
      const fbDropdown = fbBtn.closest('.fb-dropdown');
      const menu = fbDropdown.querySelector('.fb-dropdown-menu');
      if (!fbDropdown || !menu) return;

      // initialize menu hidden state to ensure consistent behavior
      menu.setAttribute('aria-hidden', 'true');
      menu.style.display = 'none';

      function openMenu() {
        fbDropdown.classList.add('open');
        fbBtn.setAttribute('aria-expanded', 'true');
        // ensure visible for browsers that may ignore CSS toggle
      ...
      function closeMenu() {
        fbDropdown.classList.remove('open');
        fbBtn.setAttribute('aria-expanded', 'false');
        menu.style.display = 'none';
        menu.setAttribute('aria-hidden', 'true');
      }

      fbBtn.addEventListener('click', (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        if (fbDropdown.classList.contains('open')) closeMenu(); else openMenu();
      });
      ...
      document.addEventListener('click', (ev) => {
        if (!fbDropdown.contains(ev.target)) closeMenu();
      });

      // keyboard support
      fbBtn.addEventListener('keydown', (ev) => {
      ...
      menu.addEventListener('keydown', (ev) => {
        ...
        if (ev.key === 'Escape') { closeMenu(); fbBtn.focus(); }
        else if (ev.key === 'ArrowDown') {
          ev.preventDefault();
          const next = links[(currentIndex + 1) % links.length]; next && next.focus();
        } else if (ev.key === 'ArrowUp') {
          ev.preventDefault();
          const prev = links[(currentIndex - 1 + links.length) % links.length]; prev && prev.focus();
        }
      });
    })();
