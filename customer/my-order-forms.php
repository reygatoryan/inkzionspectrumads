...
async function loadOrderForms() {
  ...
  try {
    ...
    if (active.length === 0 && converted.length === 0) {
      container.innerHTML = '<div class="empty-state"><div style="font-size:2.5rem;color:rgba(233,30,142,0.15);margin-bottom:0.75rem;"><i class="fas fa-file-invoice"></i></div><p style="color:#64748b;">No order forms from admin yet.</p></div>';
      return;
    }
    let html = '';
    if (active.length > 0) {
