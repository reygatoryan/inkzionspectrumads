...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .conv-avatar { width: 48px; height: 48px; border-radius: 12px; background: linear-gradient(135deg, var(--primary), #00bcd4); color: white; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 1rem; flex-shrink: 0; }
    ...
    .msg-avatar { width: 36px; height: 36px; border-radius: 10px; background: linear-gradient(135deg, var(--primary), #00bcd4); color: white; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.85rem; flex-shrink: 0; }
...
<body>
  ...
  <script>
    ...
    function showFallbackContact(message, diagnostic) {
      ...
      main.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.25rem;padding:3rem 2rem;text-align:center;height:100%;">
          <div style="width:56px;height:56px;border-radius:50%;background:rgba(233,30,140,0.1);display:flex;align-items:center;justify-content:center;font-size:1.5rem;color:#e91e8c;">
          ...
          <button onclick="autoSelectConversation()" style="margin-top:0.5rem;padding:0.6rem 1.5rem;border-radius:10px;border:none;background:linear-gradient(135deg,#e91e8c,#9c27b0);color:white;font-weight:600;font-size:0.82rem;cursor:pointer;">
    ...
    function renderConversationItem(conv) {
      ...
      const reqInfo = conv.request_type ? `<span style="font-size:0.7rem;color:#e91e8c;">${escapeHtml(conv.request_type)}</span>` : '';
      ...
      if (msg.message_type === 'text') {
        ...
        content = `<div class="msg-bubble"><div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:0.85rem;">
          <div style="font-weight:600;font-size:0.85rem;color:#0f172a;margin-bottom:0.3rem;">
            <i class="fas fa-paint-brush" style="color:#e91e8c;margin-right:0.35rem;"></i> ${escapeHtml(reqTitle)}
          </div>
          <a href="${escapeHtml(link)}" style="display:inline-block;padding:0.35rem 0.75rem;border-radius:8px;background:#e91e8c;color:white;font-size:0.75rem;font-weight:600;text-decoration:none;margin-top:0.4rem;">
    ...
    async function loadMessages(convId, silent = false) {
      ...
      if (!silent) {
        ...
        if (convData && convData.product_name) {
          const img = convData.product_image
            ...
            : `<div style="width:36px;height:36px;border-radius:8px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;font-size:1rem;color:#e91e8c;"><i class="fas fa-box"></i></div>`;
          prodBar = `<div style="display:flex;align-items:center;gap:0.75rem;padding:0.65rem 1.25rem;background:#fef2f7;border-bottom:1px solid rgba(233,30,140,0.12);">
          ...
          <div class="chat-input-area">
            <div id="file-preview" style="display:none;margin-bottom:0.5rem;padding:0.5rem;background:#f8fafc;border-radius:8px;border:1px solid var(--border-color);position:relative;">
              ...
              <div id="file-preview-content" style="display:flex;align-items:center;gap:0.5rem;">
                <i class="fas fa-file" style="color:#e91e8c;font-size:1.2rem;"></i>
      ...
      try {
        ...
        if (messages.length === 0) {
          ...
          if (hasMoreMessages) {
            html = `<div style="text-align:center;padding:0.5rem;"><button onclick="loadMoreMessages()" style="background:none;border:1px solid #e2e8f0;border-radius:8px;padding:0.4rem 1rem;color:#e91e8c;font-size:0.78rem;font-weight:500;cursor:pointer;"><i class="fas fa-chevron-up"></i> Load older messages</button></div>` + html;
          }
    ...
    function showFilePreview(file) {
      ...
      if (file.type.startsWith('image/')) {
        ...
        content.innerHTML = `<i class="fas ${icon}" style="color:#e91e8c;font-size:1.2rem;"></i><span style="font-size:0.82rem;color:var(--text-primary);">${escapeHtml(file.name)} (${(file.size / 1024 / 1024).toFixed(1)} MB)</span>`;
      }
